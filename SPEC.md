...
| 1.16 | Jun 2026 | New `type: 'notification'` branch in `/api/upload/event-image` for save-the-date and invitation marketing images. Honors EXIF orientation, resizes to a max width of 1600 px without enlargement, **no crop**, and outputs **JPG** (mozjpeg, q=85) instead of WebP — broader email-client compatibility (some clients still fail on WebP) and full original aspect ratio. The notifications tab and the wizard's Notifications step now both use this pipeline for their two image fields. The Details cover-image upload is unchanged (still cropped to 1200×400 WebP). Existing notification images uploaded under the cover pipeline need to be re-uploaded to get the un-cropped JPG variants. |
| 1.17 | Jun 2026 | Contact attribution + consolidated batch import. New `master_contacts.contributor_name TEXT NULL` (lowercase-normalized free text) tracks who supplied each contact — useful when multiple parties contribute lists for the same event. Per the design, only the **first** contributor is credited: re-importing the same email under a different contributor leaves the original attribution intact. **CSV import and Google Sheets sync moved entirely to `/admin/contacts`** (the master list); the event Contacts tab no longer has CSV upload. Both master-level batch dialogs gained a "Contributed by" field with a `<datalist>` autocomplete populated from past values. The master contacts page gained a Contributor column and a Contributor filter. The event Contacts tab now uses **Add from master** (bulk-pick) + **Add Contact** (manual one-off) only. Manual entries and checkout/RSVP rows leave `contributor_name` null — the existing `source` column already tells you the channel. |
| 1.18 | Jun 2026 | Master-list ergonomics. **Add-from-Master** sheet on each event Contacts tab gained a fourth tab — **By Contributor** — letting you pull every master contact tagged with a given contributor into the staging area (already-in-event rows are auto-excluded). **`/admin/contacts` is now sortable**: click any of Name, SMS event, SMS marketing, Source, Contributor, Added to cycle unsorted → ascending → descending; sort lives in a `?sort=` URL param alongside existing filters and pagination. **Bulk delete** lands too — a checkbox column on every row, a header indeterminate-state select-all-on-page, and an action bar that shows the count + Clear + a destructive **Delete selected**. The confirm dialog fetches an aggregate impact summary first: how many of the selected contacts hold confirmed tickets for upcoming events (across how many events), and how many have past-event attendance. Selections clear automatically when filters or pages change so you can't act on rows you can't see. |
| 1.19 | Jun 2026 | SMS delivery failure visibility. The Twilio status-callback webhook at `/api/webhooks/twilio` was already capturing `MessageStatus` + `ErrorCode` onto `invitation_logs.status` / `error_code`; this version surfaces them. New `describeTwilioError()` helper maps the codes admins actually see — 30003-30008 (landline / unreachable / blocked / filtered), 21610 (STOP), 21211/21614 (invalid number), 21408/21612 (carrier permissions) — to plain English. **`/admin/contacts/[id]` Message History** now shows the translated reason under failed-status badges. **`/admin/contacts` rows** get a small red `AlertCircle` icon next to the phone when the most recent SMS attempt for that master contact failed; hover reveals the reason. Master page does one extra batched query per visible page to fetch the latest SMS log per contact. No env var change required — Twilio signs callbacks with `TWILIO_AUTH_TOKEN`, no separate webhook secret. The Messaging Service status-callback URL must point at `/api/webhooks/twilio` in the Twilio Console for prod to receive callbacks. |
| 1.20 | Jun 2026 | Email bounce visibility, matching the SMS treatment. The Resend webhook at `/api/webhooks/resend` already mapped `email.bounced` / `email.complained` to `status='bounced'` but discarded the cause; now it pulls `bounce.subType` (falling back to `bounce.type`) onto `invitation_logs.error_code`, and tags complaints as `'SpamComplaint'`. The old `describeTwilioError()` was replaced by a channel-aware `describeDeliveryError(channel, code)` / `deliveryErrorLabel()` pair in `src/lib/delivery-errors.ts` covering both Twilio numeric codes and Resend's bounce subtypes (`MailboxFull`, `Suppressed`, `NoEmail`, `SpamComplaint`, etc.). The master-list query now fetches the most recent log per `(contact, channel)` pair in one batched call. **`/admin/contacts`** rows now show two indicators: the existing phone-column SMS marker plus a new email-column marker that fires when the contact's last email bounced; each has its own tooltip header. **`/admin/contacts/[id]` Message History** picks the right reason map based on `log.channel`. Requires the Resend webhook in the dashboard to subscribe to `email.bounced` and `email.complained` events, with `RESEND_WEBHOOK_SECRET` set in Vercel for prod. |
| 1.21 | Jun 2026 | Hide ticket inventory from the public purchase channel. The event landing page (`/e/[slug]`) and checkout form (`/e/[slug]/checkout`) no longer render the "N remaining" callout next to each tier, and the overbuy errors returned from `submitRsvp` / `createCheckoutSession` were genericized ("Not enough tickets available. Please reduce the quantity.") so the exact remaining count isn't leaked to anyone trying probe. The quantity stepper still clamps to `quantity_total - quantity_sold` so buyers can't actually overshoot. Admin views (ticket tiers page, dashboard, attendees) still show full counts. |
| 1.22 | Jun 2026 | Inactivity timeout actually fires after a browser restart. The `last_activity` cookie set by the admin-route middleware had no `maxAge`, which made it a session cookie that browsers evict on close. Supabase's own auth cookies survive that, so users with the admin page bookmarked would never cross the 12-hour window — every cold-open looked like a fresh login. Cookie is now persisted for 30 days so the stale timestamp survives a restart and the timeout check can actually trigger on the next visit. |
| 1.23 | Jun 2026 | Master-list pickers no longer cap visible matches at 100. The internal `MASTER_PICK_LIMIT` in `src/app/admin/events/[id]/contacts/actions.ts` jumped from 100 → 1000 so each filter bucket on the **Add from Master List** sheet (Search, Prior Event, Opt-In, Contributor) returns the full bucket instead of silently truncating. The four results lists also gained a sticky tri-state header checkbox — unchecked → indeterminate → checked — that bulk-stages every selectable (non-already-in-event) row in the current list; each tab passes its own `addedBy` so Prior-Event mass-adds get attributed as `event_copy` and the others as `manual`. |
| 1.24 | Jun 2026 | Friendlier marketing opt-in copy. The legal-y "By checking this box, you consent to receive text messages from {venue}..." paragraph under the SMS marketing checkbox on `/e/[slug]/checkout`, `/e/[slug]/rsvp`, and the static `/sms-opt-in` info page is replaced with a shorter, warmer pitch: "Get a text when we have an upcoming event — typically 2-4 times a year, that's it. We'll never share or sell your number. Message and data rates may apply. Reply STOP anytime. Privacy Policy". The on-record consent text saved to `sms_consents.consent_text` is unchanged. |
| 1.25 | Jun 2026 | Bounced ticket-confirmation emails are now visible on the attendees view. New `'ticket_confirmation'` value in the `message_type` enum so the initial confirmation send from the Stripe webhook can be logged distinctly from manual `'ticket_resend'` events; the webhook now inserts an `invitation_logs` row keyed by the `contacts` join row that the master sync just created plus `provider_message_id = emailResult.messageId`. Resend's existing bounce/complaint webhook updates the same row in place. The attendees page (`/admin/events/[id]/attendees`) collects the latest email-channel log per contact, builds a `bounceByEmail` lookup, and `AttendeesManager` renders an amber "Email bounced/failed" badge next to the email cell with the reason on hover (sourced from `describeDeliveryError`). Only catches future sends — bounces on tickets sent before this version are not retroactively recorded. |
| 1.26 | Jun 2026 | Detect-conflict-then-merge for master contacts. When a buyer typos their email at checkout, the master sync silently creates a second `master_contacts` row even though the admin already invited the same person under their correct address; previously the admin could see the duplicate but couldn't fix the email because `master_contacts.email UNIQUE` rejected the edit. New SECURITY DEFINER Postgres function `merge_master_contacts(from_id, to_id)` runs in one transaction: repoints `tickets.contact_id` and `invitation_logs.contact_id` from the source's join rows to the target's join rows in overlapping events; deletes the redundant source join rows; rewrites `master_contact_id` on the source's remaining join rows; OR-unions both opt-in flags onto the target; backfills target's null/empty `first_name` / `last_name` / `phone` / `contributor_name` from the source; deletes the source master. New server actions `findMasterByEmail`, `getMergePreview`, `mergeMasterContacts` in `src/app/admin/contacts/actions.ts`. The UI hook is conservative — editing a contact's email to one that's already taken opens a preview dialog showing events touched, tickets transferred, invitation/message logs transferred, and any opt-ins that would be added to the target — and on confirm routes to the surviving contact. The target's per-event `invitation_channel` always wins on overlap (admin's deliberate choice beats the buyer-typo channel). `tickets.attendee_email` is preserved as historical record. |
| 1.27 | Jun 2026 | Resend tickets, and merge now rewrites stale ticket emails. Two follow-ups to the merge feature, both targeting the same typo-then-merge scenario. **(a)** `merge_master_contacts` is replaced with a version that, after the join-row and log rewrites, also updates `tickets.attendee_email` (and `attendee_phone`) on every ticket whose join row belongs to the target post-merge and whose denormalized value still matches the source's old value — so the typo'd address stops appearing on the attendees view after the admin has fixed it at the master level. **(b)** New **Resend** row action on the attendees tab, served by `resendTickets(input)` in `src/app/admin/events/[id]/attendees/actions.ts`. The dialog pre-fills email + phone from the ticket and lets the admin override either before sending; the action bundles every confirmed/checked-in ticket in the event with the same current `attendee_email` into one `TicketConfirmationEmail` send (matching the Stripe-webhook confirmation pattern) and/or one Twilio SMS listing all ticket codes plus a verify link, applies any email/phone override across the whole bundle, and logs one `ticket_resend` row per channel to `invitation_logs` keyed by the anchor join row so Resend/Twilio webhooks can mark the send bounced/delivered later. Useful any time a buyer's contact info has changed, not just for merge cleanup. The action also runs the shared `syncMasterContactFromCheckout` helper whenever the new email differs from what the ticket had — so phone-only tickets that gain an email through resend now show up in `/admin/contacts` and the buyer is created/linked at the master level. |
| 1.28 | Jun 2026 | Add Ticket moved from the attendees tab to a per-contact row action on the event Contacts tab. Comp tickets and manual Venmo/cash/check sales always start from a known person, so the new entry point pre-fills name/email/phone from the contact. New `CreateTicketDialog` component (`src/app/admin/events/[id]/contacts/create-ticket-dialog.tsx`) hosts the same fields as the old in-place dialog — tier, quantity, comp toggle, amount paid, payment method, payment note, deliver-by-email/SMS toggles — and is remounted on each open via a prefill-derived React `key` so a second contact's prior form state can't leak. **ContactsManager** gains a `Ticket` icon button in the per-row actions cell wrapped in the existing TooltipProvider; the event contacts server page now fetches `ticket_tiers` and passes them down. **AttendeesManager** is stripped of the Add Ticket button (header + empty state), all dialog state/handlers, the `tiers` prop, the `PAYMENT_METHODS` array, and a pile of now-unused Select/Switch/Plus/Label imports. Empty-state copy directs admins to the Contacts tab for manual ticketing. |
| 1.29 | Jun 2026 | Two new admin notification flows live in the tab whose audience they target. **Ticket reminders** (event Contacts tab) ship via a new `Send Reminder` button + dialog; admin writes a custom subject + body, picks email/SMS channels, and chooses an audience scope. Default scope is *"Invited, no ticket yet"* — the server walks confirmed/checked-in tickets for this event and excludes contacts whose email or phone already appears, leaving you with just the people who still need to act; *"All contacts"* and *"Selected"* scopes are also available. New `TicketReminderEmail` template mirrors the InvitationEmail layout (heading, custom body, primary marketing image, sales CTA) but with admin-supplied headline; MMS attaches the same image. Sends are logged to `invitation_logs` as `ticket_reminder`. **Event updates** (Attendees tab) ship via a new `Send Update` button + dialog for broadcasting to confirmed/checked-in ticket holders — weather alerts, parking notes, day-of logistics, etc. New `EventUpdateEmail` template renders the event cover image, admin's custom body, and a *"View Event Details"* CTA back to the public event page. Server action dedupes by `attendee_email` so a multi-ticket buyer gets one message, not one per ticket; phone-only tickets still receive SMS. Sends are logged as `event_update`. Schema: new migration `20260607120000_message_type_reminders_updates.sql` adds `'ticket_reminder'` and `'event_update'` to the `message_type` enum and the matching TS union in `src/types/database.ts`. Follow-up patch: both flows now gate SMS sends on the relevant event-updates opt-in. **Send Update** reads `sms_consents` for this event (`consent_type='event_updates'`) — the legal record written when the buyer ticked the consent checkbox at checkout/RSVP — and only sends SMS to phones in that set; **Send Reminder** additionally requires `master_contacts.sms_opt_in_event_updates = true`. Both expose a `skippedNoOptIn` counter so the result panel tells admins how many SMS recipients were withheld; the count previews and inline hints reflect the same gate. |
| 1.30 | Jun 2026 | Fixes to v1.29. **Send Reminder no-ticket scope bug:** the "Invited, no ticket yet" count was equal to the "All contacts" count because the client-side `reminderCounts` memo never subtracted contacts whose email or phone already appears on a confirmed/checked-in ticket. The contacts server page now fetches `ticketedEmails` (lowercased) and `ticketedPhones` (digits) for this event and passes them down; the manager applies the same exclusion the server uses when actually sending. **Send Update rework:** the dual email/SMS checkbox model is replaced with a single channel-mode dropdown — *"Smart — SMS if opted in, otherwise email"* (default) or *"Email only"*. Each attendee now receives exactly one message per send instead of one per checked channel, so a buyer with both an opted-in phone and an email doesn't get spammed twice. The Attendees table picked up row-level multi-select with a tri-state header checkbox; refunded rows are not selectable; selection is intersected with the visible (search-filtered) set at every read so narrowing the search can't cause hidden rows to receive a send. The Send Update dialog now has an audience scope ("All confirmed attendees" / "Selected attendees") and opens defaulted to whichever matches the current selection. `sendEventUpdates` signature changed from `channels:{email,sms}` to `channelMode:'smart'|'email_only'`; result shape replaced with per-channel counts (`recipients`, `emailSent`, `smsSent`, `skipped`, `failed`). |
| 1.31 | Jun 2026 | Pay-with-Venmo checkout option. New per-event toggle on Details (`events.venmo_enabled boolean`, `events.venmo_handle text` default `@Anne-Olson-24`); when on, paid checkout shows a **Pay with Card / Pay with Venmo** selector. The Venmo branch skips Stripe entirely, creates pending tickets tagged with `payment_method='venmo'` and a synthetic `stripe_session_id` of the form `venmo_<uuid>`, runs the shared `syncMasterContactFromCheckout` helper inline (the Stripe path defers this to the webhook), records SMS consents identically to the card path, and redirects the buyer to a new `/e/[slug]/venmo-pending` page that displays the handle, exact amount, prescribed note (`Over Yonder Farm - [Event] - [Guest]`), an `Open Venmo` deep link (`venmo://paycharge?txn=pay&recipients=<handle>&amount=<dollars>&note=<encoded>`), and a `https://venmo.com/<handle>` desktop fallback. No confirmation email is sent until the admin verifies receipt. **Admin management:** the attendees page (`/admin/events/[id]/attendees`) gains a `PendingVenmoPanel` above the counter card showing every pending Venmo order grouped by session id — attendee name + email + phone, tier summary + count, total owed, time since ordered, a >48h amber warning badge, and `Confirm Payment` / `Cancel Order` buttons. `confirmVenmoOrder` flips the bundle to `confirmed`, bumps `quantity_sold` per tier, sends the standard `TicketConfirmationEmail`, and logs the send to `invitation_logs` as `ticket_confirmation`. `cancelVenmoOrder` flips the bundle to `cancelled` and sends a new `OrderCancelledEmail` template. The admin dashboard (`/admin`) surfaces an amber stat card with a count of pending Venmo orders across all events when at least one exists (links to the most recent affected event's attendees view). Existing Stripe checkout is unchanged; `tickets.payment_method` already supports `'venmo'` from v1.10 so no ticket-table migration is needed — the new migration (`20260622120000_event_venmo_payments.sql`) only adds the two `events` columns. |
| 1.32 | Jun 2026 | Per-event service fee pass-through. New toggle on Details (`events.pass_service_fee boolean default false`) — when on, paid card checkout adds a **Service Fee** line (between subtotal and total) covering Stripe's 2.9% + $0.30 cut, grossed-up via `fee = (subtotal * 0.029 + 0.30) / (1 - 0.029)` (cents, `Math.ceil`) in the new `src/lib/service-fee.ts` helper so the venue receives the full ticket subtotal. The fee appears as an extra `price_data` Stripe line item so the buyer is charged the gross amount; it's stored on the first ticket of the session in a new `tickets.service_fee_cents` column (integer, `>=0` check, default 0) since the fee is per-transaction not per-ticket. Venmo orders **never** carry a fee regardless of the toggle. When both methods are available the payment-method selector label changes to *"Pay with Card (+ service fee)"* vs *"Pay with Venmo"* so the buyer understands the difference. **Confirmation email** (`TicketConfirmationEmail`) gains optional `subtotalFormatted` + `serviceFeeFormatted` props — when both are passed it renders a three-row receipt (Subtotal / Service Fee / Total Paid) instead of just Amount Paid; the Stripe webhook reads `service_fee_cents` summed across the session to derive subtotal from `session.amount_total`. **Resend** does the same — bundle query now selects `service_fee_cents`, sums it, and passes the itemized props when > 0. PDF tickets are unchanged (they show the per-ticket price, not the fee). New migration `20260622130000_event_service_fee.sql` adds both columns. |
| 1.33 | Jun 2026 | Waitlist feature with separate inventory and admin-controlled offers. When all tiers are sold out and `events.waitlist_enabled` is true (defaults to TRUE for new events) the public page swaps the sold-out CTA for **Join the Waitlist** linking to a new `/e/[slug]/waitlist` page that captures first name, last name, email, phone (optional), tickets desired (1–10), and the standard SMS opt-in checkboxes. Submitting runs the shared `syncMasterContactFromCheckout` helper to upsert the master_contact (no per-event contacts join row is created — the waitlist entry is the canonical record) and inserts a `waitlist_entries` row with `position = max+1` per event. Duplicates by email return a friendly "you're already on the waitlist" confirmation. A `WaitlistConfirmationEmail` is sent best-effort. The internal **position** is sort-only and never exposed to the guest. **Admin Waitlist tab** (`/admin/events/[id]/waitlist`) lists every entry by position with status badges (`waiting` / `offered` / `purchased` / `expired` / `declined` / `skipped`), summary cards (total on list, tickets requested, waitlist tickets sold, active offers), per-row `Send Offer` (modal asks tier + quantity + hold hours, defaults to the event's `waitlist_hold_hours`), `Skip` / `Restore`, and `Re-offer` on expired/declined rows. `sendWaitlistOffer` generates a 24-byte base64url token, sets `offer_expires_at = now() + hold`, status → `offered`, and dispatches `WaitlistOfferEmail` (with offer + decline links) plus an SMS if the master is opted into event updates. The buyer's offer page (`/e/[slug]/offer/[token]`) validates token + expiry, pre-fills name/email/phone, shows a live countdown that ticks each second, the **Card Surcharge** line if the event has `pass_service_fee`, and a Card/Venmo selector when both methods are available. `acceptWaitlistOffer` creates a single ticket row with `source='waitlist'` and either kicks off a Stripe Checkout Session (with the offer's Stripe success URL) or a Venmo-pending bundle reusing the existing `venmo_<uuid>` synthetic-session pattern. Offer can be declined via `/e/[slug]/offer/[token]/decline` which flips status to `declined`. **Message Waitlist** dialog lets the admin compose a custom broadcast with recipient scope (all / waiting / expired+declined / selected), channel (email / SMS / both — SMS gated on the master's `sms_opt_in_event_updates`), and `{{first_name}}` / `{{last_name}}` / `{{event_name}}` / `{{event_date}}` merge tags rendered via the new `WaitlistCustomEmail` template; each per-recipient send is logged to `invitation_logs` as a new `message_type = 'waitlist_custom'`. Two starter templates ("No additional tickets", "More tickets coming soon") populate the form via a one-click button. **Reporting:** new `tickets.source` (`'public'` / `'waitlist'`) column distinguishes sales; the global `/admin` dashboard event rows show `N public · M waitlist` below the total when waitlist sales exist; the Attendees CSV export gains a Source column. **Schema:** new migration `20260624120000_waitlist.sql` adds `events.waitlist_enabled` (bool default TRUE), `events.waitlist_hold_hours` (int default 24, CHECK > 0), `tickets.source` (text default `'public'`, CHECK constraint), `'waitlist_custom'` to the `message_type` enum, and the `waitlist_entries` table (unique on `(event_id, master_contact_id)`, indexed by `(event_id, position)` and `(event_id, status)`) with RLS policies: public can INSERT (signup form), anon can SELECT rows that have an `offer_token` (so the public offer page can validate), team can SELECT/UPDATE/DELETE everything. |
| 1.34 | Jun 2026 | Waitlist offer expiry. Offers previously sat in `offered` status forever once their hold window lapsed — nothing in the request path flipped them to `expired`, so the admin never regained the **Re-offer** action and could not resend to the same person. New `expireStaleWaitlistOffers(eventId)` helper (`src/lib/waitlist-expiry.ts`) performs a lazy sweep: every `offered` entry whose `offer_expires_at` has passed is flipped to `expired`, and the guest is sent the previously-unused `WaitlistOfferExpiredEmail` (best-effort). It's called at the top of the admin Waitlist page load (before the entries query), so opening the tab surfaces lapsed offers as **Expired** with the existing Re-offer button in the same render. Idempotent; per spec it does **not** auto-offer the next person — expiry only unblocks the admin to re-offer manually. Chosen as lazy expiry (triggered on tab load) rather than a scheduled job since the project has no `vercel.json`/cron; the only tradeoff is the guest's expired-offer email goes out when the admin next opens the tab rather than the instant the timer hits zero. No schema change. |
| 1.35 | Jul 2026 | Admin ticket cancellation + flexible refunds. New row action on the Attendees tab (`/admin/events/[id]/attendees`) opens a Cancel/Refund dialog served by `cancelTicket` in the attendees `actions.ts`. Two intents: **cancel only** (`status → cancelled`, no money moved) or **refund** (`status → refunded`). The refund **channel is decoupled from the original payment method** — a card purchase can be refunded via Venmo / cash / check / PayPal / other (record-only) or through Stripe (`stripe.refunds.create` against the stored `stripe_payment_intent_id`); the refund amount is editable and pre-filled to the full amount paid. **Freed inventory is held for the waitlist by default:** cancelling does **not** decrement `quantity_sold`, so a sold-out event stays sold out to the public and the seat is offered from the Waitlist tab (v1.33) rather than reopening general sales — an explicit "Return this seat to public sale" checkbox decrements `quantity_sold` when the admin does want it back in the public pool. For Stripe refunds the action writes `status='refunded'` **before** calling Stripe, so the resulting `charge.refunded` webhook sees the ticket already refunded and no-ops instead of double-adjusting inventory; on a Stripe API failure the ticket is rolled back to its prior state. Optional "email the guest a cancellation notice" reuses the `OrderCancelledEmail` template (v1.31). Cancelled/refunded tickets stay visible on the tab (dimmed row, destructive Cancelled/Refunded badge, row action buttons hidden), drop out of the expected-attendance counter, and are excluded from the messaging select-all; the Status column of the CSV export now emits `Cancelled` too. **Schema:** new migration `20260723120000_ticket_cancellation_refunds.sql` adds eight columns to `tickets` — `cancelled_at`, `cancellation_reason`, `refunded_at`, `refund_method` (CHECK in stripe/cash/venmo/paypal/check/other), `refund_amount_cents` (CHECK ≥ 0), `refund_note`, `stripe_refund_id`, and `released_to_public` (bool NOT NULL default FALSE) — plus a `RefundMethod` TS union and the matching fields on the `Ticket` interface. Applied to prod via the Supabase SQL editor (the CLI can't reach the prod DB from the dev network); additive and idempotent (`ADD COLUMN IF NOT EXISTS`, constant default so no table rewrite). |
| 1.36 | Aug 2026 | Guest notes on the RSVP form, for events where one person RSVPs on behalf of a household (a wedding welcome dinner, say). Off by default and configured **per event**, not per tier — the prompt belongs to the invitation rather than to the product being reserved, a tier-level flag would have to show/hide the field as the guest switched tiers, and it could contradict itself on a multi-tier event. The event edit form gains an "Ask who's attending on the RSVP form" switch that reveals an editable question (defaults to "Who's attending with you?") and a require-an-answer toggle. On the free-event RSVP form the answer is a textarea capped at 1000 chars; the required flag is re-checked in `submitRsvp` server-side and notes are only persisted when the event's toggle is on, so a stale or crafted payload can't write them. The existing quantity input was relabelled "How many are attending?" on the RSVP path (paid checkout keeps "Quantity") and moved up to sit directly above the notes box, so "who / how many / who else" read as one cluster instead of being split by the SMS consent copy. Answers surface in the RSVP confirmation email, under the attendee name on the Attendees tab, in the attendee search (so check-in can find a child by a name that isn't on the ticket), and as a new `Guest Notes` CSV column. **Schema:** migration `20260825120000_rsvp_guest_notes.sql` adds `rsvp_guest_notes_enabled` (bool NOT NULL default false), `rsvp_guest_notes_label` (text) and `rsvp_guest_notes_required` (bool NOT NULL default false) to `events`, plus `guest_notes` (text) to `tickets`. Additive and idempotent (`ADD COLUMN IF NOT EXISTS`, constant defaults so no table rewrite); every pre-existing event's RSVP form is unchanged. |
| 1.37 | Aug 2026 | Optional email on master contacts, so SMS-first invite lists can be imported. `master_contacts.email` was `NOT NULL UNIQUE`, and the shared importer skipped every email-less row with "Missing email" — which blocked importing a phone-only list outright. **Email remains the identity key when present; phone deliberately does not become one.** Production already had 13 phone numbers shared by two or more contacts (couples and households on one mobile), so a unique index on phone would not have built, and phone-alone matching would have merged distinct people. Rows with no email are instead matched on **name + phone**, which keeps spouses on a shared number separate while still recognising a re-import of the same list — verified end-to-end: importing a phone-only list twice adds 5 then updates 5, creating no duplicates, and a later pass carrying emails attaches them to the same rows rather than making new ones. A row must still be reachable: rows with neither email nor phone are skipped, backed by a `master_contacts_reachable` CHECK. Non-empty emails are still format-validated. **Schema:** migration `20260825170000_optional_contact_email.sql` drops `master_contacts_email_key`, drops `NOT NULL` on `email`, and replaces the old raw-column unique with a partial **case-insensitive** unique index on `lower(email) WHERE email IS NOT NULL` — closing a latent bug where `Bob@x.com` and `bob@x.com` could coexist despite the app lowercasing on write (verified buildable: 0 case-duplicates in prod). Adds partial indexes on `email` and `phone`, plus the reachability CHECK. **App:** `MasterContact.email` and `PickableMasterContact.email` become `string | null`; `master-contacts-import.ts` gains the name+phone match path and keys inserted-row ids by a batch-unique key rather than by email; email drops from the required CSV headers, the Sheets column mapping, and the add/edit contact forms. Deferred to a later phase: `checkout-master-sync.ts` still requires an email, since public checkout and RSVP always collect one. |
| 1.38 | Aug 2026 | Edit an attendee in place. A guest RSVPing with the wrong party size (15 instead of 5) previously had no fix short of cancelling and re-creating the booking. New pencil row action on the Attendees tab opens an Edit dialog served by `updateTicketDetails` in the attendees `actions.ts`, covering name, email, phone, **party size**, and guest notes. **Inventory is the delicate part:** `ticket_tiers.quantity_sold` tracks the SUM of ticket quantities, so any party-size change moves it by the same delta. `adjust_quantity_sold` clamps to `[0, quantity_total]` rather than failing, so an increase past capacity would silently under-count and drift the tier out of step with its tickets — the action therefore checks remaining capacity *before* adjusting and refuses with a specific message ("Only 137 seats left in RSVP. Raising this booking to 999 needs 994."). The ticket row is written before the tier adjustment so a failed write can't leave inventory moved for a change that never landed; if the adjustment itself fails the action reports it rather than swallowing it, since the tier then needs manual attention. **Money is deliberately untouched** — changing the seat count does not recompute `amount_paid_cents`; the dialog says so inline for paid bookings and the result flags `paymentUnchanged` so the UI can warn. Collecting or returning a difference stays an explicit separate decision (Cancel/Refund, or out-of-band). Only `confirmed` / `checked_in` tickets are editable. Phone runs through the same `isValidPhone` + `normalizePhone` path as every other entry point. Verified end-to-end against a seeded 15-seat RSVP: 15→5 reports `quantityDelta=-10`, detail-only edits report `0`, over-capacity and invalid-phone and cancelled-ticket edits are all refused, and the `ticket_sum == quantity_sold` invariant holds after every case including the refused ones. **Not included:** editing `attendee_email` does not re-sync `master_contacts` — a corrected address lives on the ticket only, and the master-level fix remains the merge flow. |
...
| rsvp_guest_notes_enabled | boolean | NOT NULL default false; when true the free-event RSVP form shows a free-text guest-notes box (see §7.3) |
| rsvp_guest_notes_label | text | nullable; the question shown above that box. Falls back to "Who's attending with you?" when null |
| rsvp_guest_notes_required | boolean | NOT NULL default false; when true the RSVP can't be submitted without an answer (enforced server-side) |
...
| guest_notes | text | nullable; the guest's answer to the event's RSVP guest-notes prompt (§7.3). Null on paid checkout, on events with the toggle off, and on all tickets predating v1.36. |
...
| cancelled_at | timestamptz | nullable; set when a ticket is cancelled or refunded via the admin Cancel/Refund action |
| cancellation_reason | text | nullable; admin free-text reason; included in the guest cancellation email when one is sent |
| refunded_at | timestamptz | nullable; set when a refund is issued (Stripe) or recorded (out-of-band) |
| refund_method | text | nullable; enum-via-check `stripe \| cash \| venmo \| paypal \| check \| other`. Decoupled from `payment_method` — a card purchase can be refunded out-of-band. Only `stripe` triggers a real Stripe API refund. |
| refund_amount_cents | integer | nullable; CHECK ≥ 0; amount refunded (editable, defaults to full amount paid) |
| refund_note | text | nullable; admin free-text (e.g. "Venmo @handle, sent 7/23") |
| stripe_refund_id | text | nullable; Stripe refund id when the refund went through Stripe |
| released_to_public | boolean | NOT NULL default false; when true the cancellation decremented `quantity_sold` to return the seat to public sale. Default false keeps a sold-out event sold out (the seat is handed out from the waitlist instead). |
...
**Free events:**
1. Guest submits name on RSVP form, then party size ("How many are attending?", shown when the tier allows more than one per contact), then the optional guest-notes prompt (see below), then email and phone
...
**Guest notes (per-event, RSVP only):**
- Off by default. Enabled per event from the event edit form (`rsvp_guest_notes_enabled`), which also exposes an editable prompt (`rsvp_guest_notes_label`, defaults to "Who's attending with you?") and a require-an-answer toggle (`rsvp_guest_notes_required`).
- Configured on the **event**, not the tier: the prompt belongs to the invitation rather than to the product being reserved. A tier-level flag would have to show/hide the field as the guest switched tiers and could contradict itself on multi-tier events.
- Guest's answer is stored on `tickets.guest_notes` (max 1000 chars). The server only persists it when the event has the toggle on, and re-checks the required flag server-side — the client validation is a convenience, not the gate.
- Surfaces: echoed in the RSVP confirmation email, rendered under the attendee name on the Attendees tab, included in the attendee search (so check-in can find a child by a name that isn't on the ticket), and exported as a `Guest Notes` CSV column.
- Paid checkout is unaffected.
...
- **Cancel / Refund**: a per-row action (`cancelTicket`) removes an attendee from the list. Choose **cancel only** (no money moved, `status → cancelled`) or a **refund** (`status → refunded`) via a channel independent of how they paid — **Stripe** (real refund to the original card, requires a `stripe_payment_intent_id`) or **Venmo / cash / check / PayPal / other** (recorded as an out-of-band refund); the refund amount is editable. By default the seat is **not** returned to public sale (`quantity_sold` unchanged), so a sold-out event stays sold out and the seat is offered from the Waitlist tab; a "Return this seat to public sale" checkbox decrements `quantity_sold` to reopen it. Optionally emails the guest an `OrderCancelledEmail`. Cancelled/refunded rows stay visible (dimmed, destructive badge) but leave the expected counter and can't be checked in, resent, or messaged. For Stripe refunds the ticket is marked refunded before the Stripe call so the `charge.refunded` webhook doesn't double-adjust inventory; a failed Stripe refund rolls the ticket back.
...
| `charge.refunded` | Update ticket status to `refunded` (and decrement `quantity_sold` for tickets not already refunded) |

- Validate `Stripe-Signature` header on every webhook request using `constructEvent`
- Admin-initiated Stripe refunds (Attendees tab → Cancel/Refund, v1.35) set `status='refunded'` **before** calling `stripe.refunds.create`, so the resulting `charge.refunded` webhook is a no-op for those tickets and does not decrement `quantity_sold` a second time (the admin action already owns the inventory decision via its "return seat to public sale" choice). Refunds initiated directly from the Stripe dashboard still fall through to the webhook, which marks the ticket refunded and returns its seat to inventory.
